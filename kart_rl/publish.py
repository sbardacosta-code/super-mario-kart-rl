...
def audit():
    ...
    for name in sorted(set(names)):
        ...
        if name.startswith(('private/','.venv/','research/')) or p.suffix.lower() in {'.sfc','.smc','.rom','.bin','.state','.bk2','.zip','.pt','.pth'}:
...
def release(session):
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        notes.write_text(f'# {session} model archive\n\nAll stages, including regressions, are retained. See the classroom session report for evaluation status. These models need a compatible local ROM and validated start state; neither is included.\n')
